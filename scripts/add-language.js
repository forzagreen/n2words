...
const langCode = await rl.question('Language code (e.g., "ja", "sv", "fr-CA"): ')
const langName = await rl.question('Language name (e.g., "Japanese", "Swedish"): ')
const baseClass = await rl.question('Base class (BaseLanguage or AbstractLanguage) [BaseLanguage]: ') || 'BaseLanguage'
const negativeWord = await rl.question('Word for negative numbers (e.g., "minus", "negative") [minus]: ') || 'minus'
const separatorWord = await rl.question('Word for decimal point (e.g., "point", "dot") [point]: ') || 'point'
...
const lastImportMatch = n2wordsContent.match(/import \w+ from '.\/i18n\/[^']+\.js'\n/g)
if (lastImportMatch) {
  const lastImport = lastImportMatch[lastImportMatch.length - 1]
  const importStatement = `import ${constName} from './i18n/${fileName}.js'\n`
  n2wordsContent = n2wordsContent.replace(lastImport, lastImport + importStatement)
...
console.log('   - Fill in the cards array with number words in DESCENDING order')
