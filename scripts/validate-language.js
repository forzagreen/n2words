...
if (!existsSync(langFile)) {
  ...
  if (!content.includes('n,') && !content.includes('n]')) {
    console.warn(
      '  ⚠ No BigInt literals found (cards should use 1000n, 100n, etc.)',
    );
    warnings++;
  } else {
  ...
  if (content.includes('TODO')) {
    console.warn(
      '  ⚠ Contains TODO comments - implementation may be incomplete',
    );
    warnings++;
  }
  ...
  if (
    ...
    console.error('  ✗ Does not extend any recognized base class');
    errors++;
  } else {
...
if (!existsSync(testFile)) {
  ...
  if (scenarios.negative) console.log('  ✓ Tests negative numbers');
    ...
    console.warn('  ⚠ Missing negative number tests');
    warnings++;
  }

  if (scenarios.decimal) console.log('  ✓ Tests decimal numbers');
  else {
    console.warn('  ⚠ Missing decimal number tests');
    warnings++;
  }

  if (scenarios.large) console.log('  ✓ Tests large numbers');
  else {
    console.warn('  ⚠ Missing large number tests');
    warnings++;
  }

  if (scenarios.zero) console.log('  ✓ Tests zero');
  else {
    console.warn('  ⚠ Missing zero test');
    warnings++;
  }
}
...
if (errors === 0 && warnings === 0) {
  ...
  if (errors > 0) {
    ...
    console.warn(
      'Consider addressing the warnings for a complete implementation.',
    );
    process.exit(0);
  }
}
