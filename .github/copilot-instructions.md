...
- **Project structure:**
  - `dist/`: Browser bundle (rollup output: n2words.js with sourcemap)
...
- **Patterns & conventions** (follow these exactly):
  - Files are ESM (`package.json` includes `type: "module"`). Language implementations export classes: `export class XxLanguage extends BaseClass { ... }`. Wrapper functions are dynamically created by n2words.js factory.
...
- **How to add a language:**
  - Manual process (recommended for AI agents):
    ...
    - Export class: `export class XxLanguage extends BaseClass { ... }`. The wrapper function is auto-generated by n2words.js factory.
...
- **Build / test / lint workflows** (explicit commands):
  ...
  - Build browser bundle: `npm run web:build` (uses rollup, outputs to dist/).
...
- **Environment / runtime:**
  ...
  - Build targets: Node.js (native ESM), browsers (rollup bundle in `dist/`), TypeScript projects (type definitions).
