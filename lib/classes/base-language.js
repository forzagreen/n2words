...
/**
 * Base language converter implementing the "highest-matching card" algorithm.
 *
 * Responsibilities:
...
class BaseLanguage extends AbstractLanguage {
   ...
   * @param {Array<[bigint,string]>} cards Array of `[BigInt, string]` pairs sorted descending.
   */
  ...
  /**
   * Get the configured cards used for matching.
   * @type {Array<[bigint,string]>}
   * @readonly
   * @description Each element is `[BigInt, string]` in descending numeric order.
   */
  get cards () {
    return this.#cards
  }
