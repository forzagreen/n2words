...
export class AbstractLanguage {
   ...
   * - Passing `NaN` (as `number`) throws `Error`.
  ...
  convertToWords (value) {
    ...
    if (inputType === 'number') {
      if (Number.isNaN(value)) { throw new Error('NaN is not an accepted number.') }
      ...
      throw new TypeError(
        'Invalid variable type, expected number|string|bigint, received: ' +
          inputType
      )
    }
