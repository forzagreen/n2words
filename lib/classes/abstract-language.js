 ...
 * - Caches the whole part for languages that need it (e.g., Czech, Hebrew pluralization)
 * - Delegates whole-number wording to `convertWholePart(wholeNumber)` implemented by subclasses
 * - Converts decimals via `decimalDigitsToWords()`, preserving leading zeros and supporting per-digit mode
 *
 ...
 *
 * Note: Input validation and normalization happen at the public API boundary (n2words.js).
 * This class assumes it receives clean, pre-processed data.
 *
 * @abstract
 */
export class AbstractLanguage {
   ...
   * Merged with any default options defined by subclasses.
   * @type {Object}
   */
  options = {}

  /**
   * Return a new options object by merging language `defaults` with the
   * runtime `options` supplied by the caller.
   *
   * Merge order (later keys override earlier ones): `...defaults, ...options`.
   * Does not mutate `defaults` or `options`.
   *
   * @protected
   * @param {Object} [defaults={}] Default option values for the language.
   * @param {Object} [options={}] Runtime options supplied by the caller.
   * @returns {Object} The merged options object `{ ...defaults, ...options }`.
   */
  ...
  /**
   * Convert a whole number to words specifically for decimal contexts.
   *
   * By default, this delegates to `convertWholePart()`. Override this method
   * in languages where decimal conversion differs from whole number conversion.
   *
   ...
   *
   * @protected
   * @param {bigint} number The number to convert (from decimal portion)
   * @returns {string} The word representation for use in decimal context
   */
  convertDecimalWholePart (number) {
    return this.convertWholePart(number)
  }
   ...
   * - Otherwise delegate to `convertDecimalWholePart(digit)`
   *
  ...
  convertDigitToWord (digit) {
    ...
    return this.convertDecimalWholePart(digit)
  }
  ...
  decimalDigitsToWords (decimalString) {
    ...
    words.push(this.convertDecimalWholePart(BigInt(remainingDigits)))
   ...
   *
   * This method receives pre-validated and normalized input from the public API
   * (makeConverter in n2words.js) and assembles the final word representation.
   *
   * **Caller contract (enforced by makeConverter):**
   * - `wholeNumber` must be a non-negative BigInt (>= 0n)
   * - `decimalPart` must be a string of digits only (no leading minus, no decimal point)
   * - `isNegative` reflects the original input sign (wholeNumber is always positive)
   *
   * @public
   ...
   */
  convert (isNegative, wholeNumber, decimalPart) {
