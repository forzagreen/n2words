...
export class AbstractLanguage {
   ...
   *
   * This method normalizes the input (accepting `number | string | bigint`),
   * splits sign and fractional parts, and delegates whole-number conversion to
   * `convertWholePart(BigInt)` and fractional conversion to `decimalDigitsToWords()`.
   *
   * Note: Input validation happens at the public API boundary (n2words.js).
   * This method assumes valid input has already been checked.
   *
   * @public
  ...
  convertToWords (value) {
    // Normalize input (validation happens at public API boundary in n2words.js)
