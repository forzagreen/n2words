...
export class SlavicLanguage extends AbstractLanguage {
  ...
  integerToWords (number) {
    ...
    for (const chunkValue of chunks) {
      ...
      const [onesDigit, tensDigit, hundredsDigit] = this.extractDigits(chunkValue)
  ...
  /**
   * Splits a number string into chunks of specified size from right to left.
   *
   * Example: splitIntoChunks('1234567', 3) => [1n, 234n, 567n]
   ...
   */
  splitIntoChunks (numberString, chunkSize) {
    const chunks = []
