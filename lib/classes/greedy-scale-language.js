...
export class GreedyScaleLanguage extends AbstractLanguage {
  ...
  /**
   * Binary search for the largest scale value <= remaining.
   *
   * Since scaleWords is sorted in descending order (largest first), we search
   * for the first element where scale <= remaining.
   *
   * @private
   * @param {bigint} remaining The value to match against.
   * @returns {[bigint, string]|null} The matching [scale, word] tuple, or null.
   */
  #findMatchingScale (remaining) {
    const scales = this.scaleWords
    let lo = 0
    let hi = scales.length - 1

    while (lo < hi) {
      const mid = (lo + hi) >> 1
      if (scales[mid][0] > remaining) {
        lo = mid + 1
      } else {
        hi = mid
      }
    ...
    return scales[lo][0] <= remaining ? scales[lo] : null
  }
  ...
  decomposeInteger (integerPart) {
    ...
    do {
      const match = this.#findMatchingScale(remaining)
      if (!match) break
  ...
  integerToWords (integerPart) {
    // Zero is a special case - return zeroWord directly
    if (integerPart === 0n) {
