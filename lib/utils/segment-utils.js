...
/**
 * Splits a number string into groups of 4 digits (myriad-based grouping).
 *
 * This is the East Asian numbering system (Chinese, Japanese, Korean)
 * where the base unit is 万 (10,000) rather than 1,000.
 *
 * Groups: 万 (10^4), 亿/억 (10^8), 兆/조 (10^12), 京/경 (10^16), etc.
 *
 * @param {string} numberString The number as a string.
 * @returns {bigint[]} Array of BigInt segments from highest to lowest scale.
 *
 * @example
 * groupByFours('123456789') => [1n, 2345n, 6789n]
 * // Represents: 1 亿 + 2345 万 + 6789
 */
export function groupByFours (numberString) {
  const segments = []
  const len = numberString.length
  const segmentSize = 4
...
/**
 * Extracts ones, tens, hundreds, and thousands place values from a 4-digit segment.
 *
 * Given a number 0-9999, returns an array of its place values
 * in order: [ones, tens, hundreds, thousands].
 *
 * @param {bigint} segment Value between 0n and 9999n.
 * @returns {bigint[]} Array of [ones, tens, hundreds, thousands] as BigInts.
 *
 * @example
 * placeValuesFour(4567n) => [7n, 6n, 5n, 4n]
 * // 4567 = 4 thousands + 5 hundreds + 6 tens + 7 ones
 */
export function placeValuesFour (segment) {
  const ones = segment % 10n
  const tens = (segment / 10n) % 10n
  const hundreds = (segment / 100n) % 10n
  const thousands = segment / 1000n
