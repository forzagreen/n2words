    ...
    if (tens > 0) {
      if (tens < 20) {
        if (tens == 2 && Math.trunc(hundreds) == 0 && groupLevel > 0) {
          let pow = Math.trunc(Math.log10(Number(this.number)));
          if (pow % 3 == 0 && this.number > 10 && this.number == 2 * Math.pow(10, pow)) {
