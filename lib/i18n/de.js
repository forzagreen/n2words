...
export class German extends CardMatchLanguage {
   ...
   * - Implicit "eins": `merge({ 'eins': 1n }, { 'hundert': 100n })` → `{ 'einhundert': 100n }`
   * - Compound words without separators (e.g., "einundzwanzig" = 21)
   ...
   * - Reordering of tens and units: `merge({ 'zwanzig': 20n }, { 'eins': 1n })` → `{ 'einundzwanzig': 21n }`
   *
   ...
   *
   * @example
   * merge({ 'eins': 1n }, { 'hundert': 100n }); // { 'einhundert': 100n }
   * merge({ 'zwanzig': 20n }, { 'drei': 3n }); // { 'dreiundzwanzig': 23n }
   */
