 ...
 */
export class Spanish extends CardMatchLanguage {
  negativeWord = 'menos'
   ...
   * - Implicit "uno": `merge({ 'uno': 1n }, { 'mil': 1000n })` → `{ 'mil': 1000n }`
   * - "y" (and) between tens and units: `merge({ 'veinte': 20n }, { 'uno': 1n })` → `{ 'veinte y uno': 21n }`
   ...
   *
   * @example
   * merge({ 'uno': 1n }, { 'mil': 1000n }); // { 'mil': 1000n }
   * merge({ 'veinte': 20n }, { 'tres': 3n }); // { 'veinte y tres': 23n }
   */
...
export default function floatToCardinal (value, options = {}) {
  return new Spanish(options).floatToCardinal(value)
}
