...
export class Japanese extends AbstractLanguage {
  ...
  /**
   * Initializes the Japanese converter.
   *
   * @param {Object} options
   * @param {string} [options.negativeWord='マイナス'] Word for negative numbers.
   * @param {string} [options.separatorWord='点'] Word separating whole and decimal parts (dot/point).
   * @param {string} [options.zero='零'] Word for 0. Alternative: '〇' (maru).
   */
  constructor (options) {
    options = Object.assign({
      ...
      separatorWord: '点',
      zero: '零',
      spaceSeparator: '' // Japanese doesn't use spaces between characters
    }, options)
  ...
  /**
   * Converts decimal digits to Japanese by reading each digit individually.
   * Japanese decimal parts are read digit-by-digit, not as numbers.
   *
  ...
  decimalToCardinal (decimalString) {
    ...
    for (const digit of decimalString) {
      words.push(this.digits[Number(digit)] || this.zero)
    }
    return words
  }
  ...
  convertGroup (num, isTopGroup = false) {
    ...
    if (thousands > 0n) {
      ...
      if (thousands === 1n) {
        ...
        result += this.digits[Number(thousands)] + '千'
      }
    }
    ...
    if (hundreds > 0n) {
      ...
      if (hundreds === 1n) {
        ...
        result += this.digits[Number(hundreds)] + '百'
      }
    }
    ...
    if (tens > 0n) {
      ...
      if (tens === 1n) {
        ...
        result += this.digits[Number(tens)] + '十'
      }
    }

    // Ones
    if (ones > 0n) {
      result += this.digits[Number(ones)]
    }
 ...
 *
 * @example
 * floatToCardinal(42, { lang: 'ja' }); // '四十二'
 * floatToCardinal(1000, { lang: 'ja' }); // '千'
 * floatToCardinal(10000, { lang: 'ja' }); // '一万'
 */
export default function floatToCardinal (value, options = {}) {
  return new Japanese(options).floatToCardinal(value)
}
