...
export class Dutch extends GreedyScaleLanguage {
  ...
  toCardinalWords (value) {
    if (value >= 1100n && value < 10_000n && !this.noHundredPairs) {
      const high = value / 100n
      const low = value % 100n
      if (high % 10n !== 0n) {
        let result = super.toCardinalWords(high) + 'honderd'
        if (low) {
          result +=
            (this.includeOptionalAnd ? ' en ' : ' ') + super.toCardinalWords(low)
        }
        return result
      }
    }
    return super.toCardinalWords(value)
  }
}
