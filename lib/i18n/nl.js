...
export class Dutch extends CardMatchLanguage {
  ...
  constructor ({ includeOptionalAnd = false, noHundredPairs = false, accentOne = true } = {}) {
    ...
    if (!this.accentOne) {
      this.cards[this.cards.length - 2][1] = 'een'
    }
  }
   ...
   * - Implicit "één": `merge({ 'één': 1n }, { 'duizend': 1000n })` → `{ 'duizend': 1000n }`
   * - Reversed digit order for tens + units: `merge({ 'twintig': 20n }, { 'één': 1n })` → `{ 'ééntwintig': 21n }`
   ...
   *
   * @example
   * merge({ 'één': 1n }, { 'duizend': 1000n }); // { 'duizend': 1000n }
   * merge({ 'twintig': 20n }, { 'drie': 3n }); // { 'drieentwintig': 23n }
   */
  merge (current, next) {
 ...
 *
 * @example
 * floatToCardinal(1); // 'één' (default accent)
 * floatToCardinal(1, { accentOne: false }); // 'een'
 * floatToCardinal(21); // 'eenentwintig' (no accent in compounds)
 * floatToCardinal(42); // 'tweeenveertig'
 * floatToCardinal('1.5'); // 'één komma vijf'
 */
