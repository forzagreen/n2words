 ...
 * - Phonetic contractions (removes duplicate vowels)
 * - Accent on final "tre" in compound numbers (ventitré)
 * - Special handling for "uno" and gender agreement
 * - Complex composition patterns for large numbers
 *
 * Features:
 * - Vowel elision (e.g., "ventotto" not "ventiotto")
 * - Accentuation rules for "tre" (ventitré, trentatré)
 * - Exponent-based large number naming (milione, miliardo, trilione)
 * - Custom word construction for hundreds and thousands
 *
 * Note: This implementation uses custom algorithms rather than
 * standard BaseLanguage patterns (see TODO comment).
 */
export class IT extends BaseLanguage {
  cardinalWords = [
  ...
  /**
   * Initializes the Italian converter with language-specific options.
   *
   * @param {Object} options
   * @param {string} [options.negativeWord='meno'] Word for negative numbers (less).
   * @param {string} [options.separatorWord='virgola'] Word separating whole and decimal parts (comma).
  ...
  thousandsToCardinal (number) {
    ...
    const prefix = thousands === 1 ? 'mille' : this.toCardinal(thousands) + 'mila'
...
export default function floatToCardinal (value, options = {}) {
  return new IT(options).floatToCardinal(value)
}
