 ...
 */
export class RU extends SlavicLanguage {
  /** @type {object} */
  ones = {
    1: 'один',
    2: 'два',
  ...
  /**
   * Initializes the Russian converter with grammar support.
   *
   * Russian has complex pluralization rules requiring three forms for each scale word:
   * - Form 1: singular (тысяча = 1 thousand)
   * - Form 2: few (тысячи = 2-4 thousands)
   * - Form 3: many (тысяч = 5-20, 25-30, etc.)
   *
   * Gender agreement is handled via separate `ones` and `onesFeminine` tables
   * (e.g., один vs одна for masculine vs feminine genders).
   *
   * @param {Object} options
   * @param {string} [options.negativeWord='минус'] Word for negative numbers (minus).
   * @param {string} [options.separatorWord='запятая'] Word separating whole and decimal parts (comma).
   * @param {string} [options.zero='ноль'] Word for the digit 0 (zero).
...
export default function floatToCardinal (value, options = {}) {
  return new RU(options).floatToCardinal(value)
}
