...
export default function(options) {
  ...
  const oldToCardinal = this.toCardinal;
  this.toCardinal = value => {
    if (value >= 1100 && value < 10000 && !localOptions.noHundredPairs) {
      const high = Math.floor(value / 100);
      const low = value % 100;
      if (high % 10 !== 0) {
        return oldToCardinal(high) + 'honderd' +
          (low ? (localOptions.includeOptionalAnd ? ' en ' : ' ') + oldToCardinal(low) : '');
      }
    }
    return oldToCardinal(value);
  };
}
