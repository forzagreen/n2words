 ...
 */
export class Swedish extends CardMatchLanguage {
  negativeWord = 'minus'
   ...
   * - Implicit "ett": `merge({ 'ett': 1n }, { 'hundra': 100n })` → `{ 'hundra': 100n }`
   * - Hyphenated compounds: `merge({ 'tjugo': 20n }, { 'tre': 3n })` → `{ 'tjugo-tre': 23n }`
   * - "och" after hundreds: `merge({ 'hundra': 100n }, { 'ett': 1n })` → `{ 'hundra och ett': 101n }`
   * - "en" for millions: `merge({ 'ett': 1n }, { 'miljon': 1000000n })` → `{ 'en miljon': 1000000n }`
   *
   ...
   *
   * @example
   * merge({ 'ett': 1n }, { 'hundra': 100n }); // { 'hundra': 100n }
   * merge({ 'tjugo': 20n }, { 'tre': 3n }); // { 'tjugo-tre': 23n }
   */
 ...
 *
 * @example
 * floatToCardinal(42); // 'fyrtio-två'
 * floatToCardinal('1.5'); // 'ett komma fem'
 */
export default function floatToCardinal (value, options = {}) {
  return new Swedish(options).floatToCardinal(value)
}
