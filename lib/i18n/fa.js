...
export class Farsi extends AbstractLanguage {
  ...
  toCardinalWords (number) {
    ...
    if (number > 100n && number < 1000n) {
      ...
      const tail = this.toCardinalWords(number - xhundred)
    ...
    if (number > 1000n && number < 1_000_000n) {
      ...
      const namedThousandMultiplier =
        (thousandMultiplier === 1n
          ? ''
          : this.toCardinalWords(thousandMultiplier)) +
        ' ' +
        this.namedNumbers[1000]
      const tailNumber = number - thousandMultiplier * 1000n
      const tail = tailNumber === 0n ? '' : ' ' + this.toCardinalWords(tailNumber)
    ...
    if (number >= 1_000_000n) {
      ...
      const namedMillion =
        this.toCardinalWords(millionMultiplier) + ' ' + this.namedNumbers[1_000_000]
      const tailNumber = number - millionMultiplier * 1_000_000n
      const tail = tailNumber === 0n ? '' : ' و ' + this.toCardinalWords(tailNumber)
