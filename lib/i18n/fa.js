 ...
 */
export class FA extends AbstractLanguage {
  namedNumbers = {
    0: 'صفر',
    1: 'یک',
    2: 'دو',
  ...
  /**
   * Initializes the Persian converter with language-specific options.
   *
   * @param {Object} options
   * @param {string} [options.negativeWord='منفى'] Word for negative numbers.
   * @param {string} [options.separatorWord='ممیّز'] Word separating whole and decimal parts.
  ...
  toCardinal (number) {
    ...
    if (number > 100n && number < 1000n) {
      ...
      const tail = this.toCardinal(number - xhundred)
    ...
    if (number > 1000n && number < 1_000_000n) {
      ...
      const tail = tailNumber === 0n ? '' : ' ' + this.toCardinal(tailNumber)
    ...
    if (number >= 1_000_000n) {
      ...
      const tail = tailNumber === 0n ? '' : ' و ' + this.toCardinal(tailNumber)
...
export default function floatToCardinal (value, options = {}) {
  return new FA(options).floatToCardinal(value)
}
