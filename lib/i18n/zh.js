...
export class Chinese extends GreedyScaleLanguage {
  ...
  decimalToCardinal (decimalString) {
    ...
    for (let i = 0; i < decimalString.length; i++) {
      ...
      words.push(this.toCardinalWords(digitValue))
    }
    return words
  }
}
 ...
 * @param {Object} [options] Conversion options (see Chinese class options).
 * @param {boolean} [options.formal=true] Use formal/financial numerals (壹贰叁) vs. common numerals (一二三).
 ...
 *
 * @example
 * floatToCardinal(42); // '肆拾贰' (formal style)
 * floatToCardinal(42, { formal: false }); // '四十二' (common style)
 */
