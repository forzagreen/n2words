...
export class N2WordsFR extends BaseLanguage {
  /**
   * Initializes the French converter.
   *
   * @param {Object} options
   * @param {string} [options.negativeWord='moins'] Word for negative numbers.
   * @param {string} [options.separatorWord='virgule'] Word separating whole and decimal parts.
   * @param {string} [options.zero='zéro'] Word for 0.
   * @param {string} [options._region='FR'] Region variant: 'FR' or 'BE'.
   * @param {boolean} [options.withHyphenSeparator=false] Use hyphens instead of spaces.
   */
  constructor (options) {
  ...
  merge (currentPair, nextPair) {
    ...
    if (currentNumber === 1n) {
      ...
      // Remove trailing 's' from multiples of 80 or round hundreds when followed by smaller numbers
      if (
      ...
      // Add 's' to hundreds in certain conditions
      if (
    ...
    // Hyphenate or use "et" for compound tens
    if (nextNumber < currentNumber && currentNumber < 100) {
 ...
 * @param {Object} [options] Conversion options (see N2WordsFR class).
