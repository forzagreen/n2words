 ...
 *
 * CardMatchLanguage with inline Norwegian merge rules:
 ...
 */
export class Norwegian extends CardMatchLanguage {
  negativeWord = 'minus'
   ...
   * - Implicit \"en\": `merge({ 'en': 1n }, { 'hundre': 100n })` → `{ 'hundre': 100n }`
   * - Hyphenation for compound tens: `merge({ 'tjue': 20n }, { 'en': 1n })` → `{ 'tjue-en': 21n }`
   * - \"og\" (and) after hundreds: `merge({ 'hundre': 100n }, { 'en': 1n })` → `{ 'hundre og en': 101n }`
   ...
   *
   * @example
   * merge({ 'en': 1n }, { 'hundre': 100n }); // { 'hundre': 100n }
   * merge({ 'tjue': 20n }, { 'tre': 3n }); // { 'tjue-tre': 23n }
   */
 ...
 *
 * @example
 * floatToCardinal(42); // 'førti-to'
 * floatToCardinal('1.5'); // 'en komma fem'
 */
export default function floatToCardinal (value, options = {}) {
  return new Norwegian(options).floatToCardinal(value)
}
