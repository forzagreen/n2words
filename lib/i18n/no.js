import ScandinavianLanguage from '../classes/scandinavian-language.js'

/**
 * Norwegian language converter.
 *
 * Inherits from ScandinavianLanguage shared patterns:
 * - Hyphenation for compound numbers (e.g., "tjueen")
 * - "og" (and) for hundreds combinations
 * - Comma separation for non-magnitude additions
 * - Implicit '1' before tens and magnitudes
 * - Space separators for large numbers
 */
export class Norwegian extends ScandinavianLanguage {
  /**
   * Initializes the Norwegian converter with language-specific options.
   *
   * @param {Object} options Configuration options.
   * @param {string} [options.negativeWord='minus'] Word for negative numbers.
   * @param {string} [options.separatorWord='komma'] Word separating whole and decimal parts.
   * @param {string} [options.zero='null'] Word for 0.
   */
  constructor (options) {
    options = Object.assign(
      {
        negativeWord: 'minus',
        separatorWord: 'komma',
        zero: 'null'
      },
      options
    )

    super(options, [
      [1_000_000_000_000_000_000_000_000_000_000_000n, 'quintillard'],
      [1_000_000_000_000_000_000_000_000_000_000n, 'quintillion'],
      [1_000_000_000_000_000_000_000_000_000n, 'quadrillard'],
      [1_000_000_000_000_000_000_000_000n, 'quadrillion'],
      [1_000_000_000_000_000_000_000n, 'trillard'],
      [1_000_000_000_000_000_000n, 'trillion'],
      [1_000_000_000_000_000n, 'billard'],
      [1_000_000_000_000n, 'billion'],
      [1_000_000_000n, 'millard'],
      [1_000_000n, 'million'],
      [1000n, 'tusen'],
      [100n, 'hundre'],
      [90n, 'nitti'],
      [80n, 'åtti'],
      [70n, 'sytti'],
      [60n, 'seksti'],
      [50n, 'femti'],
      [40n, 'førti'],
      [30n, 'tretti'],
      [20n, 'tjue'],
      [19n, 'nitten'],
      [18n, 'atten'],
      [17n, 'sytten'],
      [16n, 'seksten'],
      [15n, 'femten'],
      [14n, 'fjorten'],
      [13n, 'tretten'],
      [12n, 'tolv'],
      [11n, 'elleve'],
      [10n, 'ti'],
      [9n, 'ni'],
      [8n, 'åtte'],
      [7n, 'syv'],
      [6n, 'seks'],
      [5n, 'fem'],
      [4n, 'fire'],
      [3n, 'tre'],
      [2n, 'to'],
      [1n, 'en'],
      [0n, 'null']
    ])
  }
} /**
 * Converts a number to Norwegian cardinal (written) form.
 *
 * @param {number|string|bigint} value The number to convert.
 * @param {Object} [options] Conversion options (see NO class).
 * @returns {string} The number expressed in Norwegian words.
 * @throws {TypeError} If value is NaN or invalid type.
 * @throws {Error} If value is an invalid number string.
 *
 * @example
 * floatToCardinal(42, { lang: 'no' }); // 'førtito'
 * floatToCardinal(1.5, { lang: 'no' }); // 'en komma fem'
 */
export default function floatToCardinal (value, options = {}) {
  return new Norwegian(options).floatToCardinal(value)
}
