...
export class N2WordsHU extends N2WordsBase {
  ...
  tensToCardinal(number) {
    if (this.getValueFromCard(number)) {
      return this.getValueFromCard(number);
    } else {
      const tens = Math.floor(number / 10);
      const units = number % 10;
      return this.getValueFromCard(tens * 10) + this.toCardinal(units);
    }
  }
  ...
  bigNumberToCardinal(number) {
    ...
    return prefix + this.getValueFromCard(exp) + postfix;
  }
  ...
  toCardinal(number, zero = this.zero) {
    ...
    if (number == 0) {
      ...
      words = this.getValueFromCard(number);
    } else if (number < 100) {
