...
function floatToCardinal (value, options = {}) {
  ...
  // Extract language with minimal string operations
  let lang = options.lang

  // Fast path: no lang specified, use default 'en'
  if (lang === undefined) {
    return dict.en(value, options)
  }

  // Convert to string only if needed (faster than always converting)
  if (typeof lang !== 'string') {
    lang = String(lang)
  }

  // Direct lookup (most common case - exact match)
  let converter = dict[lang]
  if (converter !== undefined) {
    return converter(value, options)
  }

  // Progressive fallback for regional variants: use lastIndexOf for efficiency
  // e.g. 'fr-BE-XX' -> try 'fr-BE' -> try 'fr'
