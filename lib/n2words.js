...
function makeConverter (LanguageClass) {
  ...
  return function convertToWords (value, options) {
    ...
    if (inputType === 'bigint') {
      // BigInt: no decimals, handle sign directly
      if (value < 0n) {
        isNegative = true
        wholeNumber = -value
      } else {
      ...
      // Number or string: normalize to string, then parse
      let str = inputType === 'number' ? value.toString() : value.trim()
      ...
      if (decimalPointIndex === -1) {
        wholeNumber = BigInt(str)
      } else {
        const wholePartString = str.slice(0, decimalPointIndex) || '0'
        wholeNumber = BigInt(wholePartString)
        decimalPart = str.slice(decimalPointIndex + 1)
      }
    }
