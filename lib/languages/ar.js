 ...
 * Note: Arabic uses "و" (and) attached to the following word, not separated by space.
 */
function segmentToWords (groupNumber, groupLevel, fullNumber, ones) {
  ...
  if (hundredsDigit > 0) {
    if (tensValue === 0 && hundredsDigit === 2) {
      ...
      if (hundredsWord) {
        result = hundredsWord
        if (tensValue !== 0) {
          result += ' ' + AND // "مائة و" -> space before و, attached to next word
        }
      }
    }
  }
  ...
  if (tensValue > 0) {
    if (tensValue < 20) {
      ...
      if (onesDigit > 0) {
        result += ones[onesDigit - 1] + ' ' + AND // "ستة و" -> attached to tens word
      }
...
function integerToWords (n, options) {
  ...
  // Join groups with و (and) - attached to following word
  return groups.join(' ' + AND).replace(/\s+/g, ' ').trim()
}
