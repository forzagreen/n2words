 ...
 *   When true, numbers use feminine forms (واحدة instead of واحد).
 *   Affects all number words and grammatical agreement.
 * @example
 * // Masculine (default)
 * const converter = new Arabic();
 * converter.convertToWords(1); // 'واحد'
 *
 * // Feminine
 * const femConverter = new Arabic({ feminine: true });
 * femConverter.convertToWords(1); // 'واحدة'
 *
 * // Custom negative word
 * const customConverter = new Arabic({ negativeWord: 'سالب' });
 * customConverter.convertToWords(-5); // 'سالب خمسة'
 */
...
export class Arabic extends AbstractLanguage {
  ...
  // Group magnitude words (thousand, million, etc.)
  ...
  // Special dual forms (Arabic has singular, dual, plural)
  ...
  // Gender-specific ones (1-19) with masculine/feminine forms
  ones = {
    masculine: [
  ...
  constructor (options = {}) {
    ...
    this.negativeWord = this.options.negativeWord

    delete this.options.negativeWord
  }

  /**
   * Gets the appropriate ones array based on gender option.
   * @returns {Array<string>} The selected ones array.
   */
  get selectedOnes () {
