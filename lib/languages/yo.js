...
// Precompute 0-99
const UNDER_100 = new Array(100)
for (let i = 0; i < 100; i++) {
  UNDER_100[i] = buildUnder100(i)
}
...
function convertHundreds (n) {
  ...
  if (remainder > 0) {
    result += ' ' + AND + ' ' + UNDER_100[remainder]
  }
...
function integerToWords (n) {
  ...
  if (remaining >= 1000n) {
    ...
    if (thousands === 1n) {
      ...
      parts.push(THOUSAND + ' ' + UNDER_100[Number(thousands)])
    } else {
  ...
  if (remaining > 0n) {
    if (remaining < 100n) {
      if (parts.length > 0) {
        parts.push(AND + ' ' + UNDER_100[Number(remaining)])
      } else {
        parts.push(UNDER_100[Number(remaining)])
      }
    } else {
      if (parts.length > 0) {
