...
function buildLargeNumberWords (n) {
  ...
  // Find the first non-zero segment (processed last, needs "e" logic)
  ...
  for (let i = segments.length - 1; i >= 0; i--) {
    ...
    const segmentWord = SEGMENTS[segmentNum]
    ...
    if (result && isLastSegment && prevWasScale && !startsWithHundreds(segmentWord)) {
      result += ' e'
    }
