...
/**
 * Chinese Traditional language converter.
 *
 * Features:
 ...
 */
export class ChineseTraditional extends GreedyScaleLanguage {
  negativeWord = '負'
  ...
  /**
   * Initializes the Chinese Traditional converter.
   *
   * @param {ChineseTraditionalOptions} [options={}] Configuration options.
   */
  constructor (options = {}) {
    super()
...
/**
 * Converts a number to Chinese Traditional cardinal (written) form.
 *
 * @param {number|string|bigint} value The number to convert.
 * @param {ChineseTraditionalOptions} [options] Conversion options.
 * @returns {string} The number expressed in Chinese Traditional words.
 * @throws {TypeError} If value is an invalid type.
 * @throws {Error} If value is NaN or an invalid number string.
 *
 * @example
 * convertToWords(42); // '肆拾貳' (formal style)
 * convertToWords(42, { formal: false }); // '四十二' (common style)
 */
export default function convertToWords (value, options = {}) {
  return new ChineseTraditional(options).convertToWords(value)
}
