...
export class Persian extends AbstractLanguage {
  ...
  /** Converts whole number using categorized word tables. */
  integerToWords (number) {
    ...
    // 1-9
    if (number <= 9n) {
      return this.onesWords[number]
    }

    // 10-19
    if (number <= 19n) {
      return this.teensWords[number]
    }
    ...
    // 100-999
    if (number < 1000n) {
      const hundreds = 100n * (number / 100n)
    ...
    const remainder = number % 1_000_000n
