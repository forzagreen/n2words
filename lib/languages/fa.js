...
export class Persian extends AbstractLanguage {
  ...
  /**
   * Lookup table for Persian number words.
   * Maps numeric values to their word representations.
  ...
  integerToWords (number) {
    if (this.numberWords[number] && number !== 1_000_000n) {
    ...
    if (number > 20n && number < 100n) {
      ...
      return `${this.numberWords[xten]} و ${this.numberWords[xone]}`
    }

    if (number > 100n && number < 1000n) {
      const xhundred = 100n * (number / 100n)
      const tail = this.integerToWords(number - xhundred)
      return `${this.numberWords[xhundred]} و ${tail}`
    }
    ...
    if (number > 1000n && number < 1_000_000n) {
      ...
      const namedThousandMultiplier =
        ...
        ' ' +
        this.numberWords[1000]
      const tailNumber = number - thousandMultiplier * 1000n
      const tail = tailNumber === 0n ? '' : ' ' + this.integerToWords(tailNumber)
    ...
    if (number >= 1_000_000n) {
      ...
      const namedMillion =
        this.integerToWords(millionMultiplier) + ' ' + this.numberWords[1_000_000]
      const tailNumber = number - millionMultiplier * 1_000_000n
      const tail = tailNumber === 0n ? '' : ' و ' + this.integerToWords(tailNumber)
