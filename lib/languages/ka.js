 ...
 *
 * Georgian-specific rules (handled in precomputation):
...
const SEGMENTS_FULL = new Array(1000)
const SEGMENTS_STEM = new Array(1000)

for (let i = 0; i < 1000; i++) {
  const result = buildSegment(i)
  SEGMENTS_FULL[i] = result.full
  SEGMENTS_STEM[i] = result.stem
}
...
function integerToWords (n) {
  ...
  if (n < 1_000_000n) {
    ...
    if (thousands === 1) {
      ...
      const thousandsPart = SEGMENTS_STEM[thousands]
      result = thousandsPart + ' ' + (remainder > 0 ? THOUSAND_STEM : THOUSAND)
    }

    if (remainder > 0) {
      result += ' ' + SEGMENTS_FULL[remainder]
    }
...
function buildLargeNumberWords (n) {
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (segment !== 0) {
      if (scaleIndex === 0) {
        // Units (no scale)
        parts.push(SEGMENTS_FULL[segment])
        ...
        if (segment === 1) {
          ...
          parts.push(SEGMENTS_STEM[segment] + ' ' + thousandWord)
        }
      } else {
        // Million and above
        ...
        if (segment === 1) {
          ...
          parts.push(SEGMENTS_FULL[segment] + ' ' + scaleWord)
        }
      }
    }
