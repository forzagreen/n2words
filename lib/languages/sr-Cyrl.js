...
/**
 * Converts a number to Serbian Cyrillic cardinal (written) form.
 *
 * @param {number|string|bigint} value The number to convert.
 * @param {SerbianCyrillicOptions} [options={}] Configuration options.
 * @param {boolean} [options.feminine=false] Use feminine forms for numbers.
 * @returns {string} The number expressed in Serbian Cyrillic words.
 * @throws {TypeError} If value is an invalid type.
 * @throws {Error} If value is NaN or an invalid number string.
 */
export default function convertToWords (value, options = {}) {
  return new SerbianCyrillic(options).convertToWords(value)
}
