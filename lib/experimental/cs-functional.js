...
// Ones words (feminine form - only 2 differs)
const ONES_FEM = ['', 'jedna', 'dvě', 'tři', 'čtyři', 'pět', 'šest', 'sedm', 'osm', 'devět']
  ...
  for (let i = 0; i < segments.length; i++) {
    ...
    if (segment !== 0n) {
      if (scaleIndex === 0) {
        ...
        parts.push(SEGMENTS_WITH_HUNDREDS[Number(segment)])
      } else {
        // Scale word needed
